...
现在为 IFU 实现了 AXI4-Lite，但目前完成一次取指需要好多个周期，这会影响到流水线的正常运行，使得流水线每一级需要多个周期才能完成。并且跳转指令也出了问题：如果当前处于译码阶段的指令是跳转指令，那么 dnpc 会被放入 PC 寄存器，到下一条指令被读出后才会被读进 IDReg，这太迟了，应该冲刷掉 IDReg 里的指令，即判断当前是否有指令跳转，如果有就把
...
可以新建一个 fake_sram 模块，并用 AXI4-Lite 接口包装，该模块内调用 DPIC 访问内存。这个后面再做，先写一个能跑的版本

不应该修改 DPIC 使用的 verilog 文件的，应该在外层模块用 AXI4-Lite

AMBA 手册里有 AXI4 各握手信号的依赖关系图，可以按照那个进行
