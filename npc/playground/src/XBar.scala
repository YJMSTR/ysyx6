...
class XBar extends Module {
  ...
  // out(0): uart
  // out(1): sram
  val empty_slave_in0 = Module(new empty_axi4_master)
  val empty_slave_in1 = Module(new empty_axi4_master)
  val empty_slave_in2 = Module(new empty_axi4_master)
  val empty_master_out = Module(new empty_axi4_slave)
  ...
  switch(state_r) {
    is (r_idle) {
      when (io.axi4in.arvalid) {
        when(io.axi4in.araddr >= MEM_BASE.U(XLEN.W) && io.axi4in.araddr < MEM_BASE.U(XLEN.W) + MEM_SIZE.U(XLEN.W)) {
          state_r := r_sram_wait
          // 暂时把 RTC 转发给 SRAM
